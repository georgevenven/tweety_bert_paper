...
def plot_umap_projection(model, device, data_dir="test_llb16",
    ...
    for i, (data, _, ground_truth_label) in enumerate(data_loader):
        ...
        if len(predictions_arr) > samples:
            break 
    ...
    # Check for NaNs
    nan_exists = np.isnan(predictions).any()
    print(f"Are there any NaN values in predictions? {'Yes' if nan_exists else 'No'}")

    ground_truth_labels = np.concatenate(ground_truth_labels_arr, axis=0)
    
    # dims is 1 if no windowing is utilized
    samples, dims = ground_truth_labels.shape
    ...
    if compute_svm:
        ...
        plt.scatter(embedding_scaled[:, 0], embedding_scaled[:, 1], c=[label_to_color[lbl] for lbl in ground_truth_labels], edgecolors='k', s=20)

    else:
        plt.scatter(embedding_outputs[:, 0], embedding_outputs[:, 1], c=[label_to_color[lbl] for lbl in ground_truth_labels], s=10, alpha=.5)

    # if raw_spectogram:
    #     plt.title(f'UMAP of Spectogram with SVM Decision Boundary', fontsize=14)
    # else:
    #     plt.title(f'UMAP Projection with SVM (Layer: {layer_index}, Key: {dict_key})', fontsize=14)
...
def plot_pca_projection(model, device, data_dir="test_llb16",
    ...
    for i, (data, _, ground_truth_label) in enumerate(data_loader):
        ...
        # rework this 
            ...
            # data shape [0] is the number of batches, 
            predictions = data.reshape(batches, num_times, time_bins_per_umap_point, features)
    ...
    print(f"ground_truth_labels shape {ground_truth_labels.shape}")

    # to allow sci notation 
    # Sample randomly from the arrays
    if samples > len(predictions):
        samples = len(predictions)
    else:
        predictions = predictions[:samples]
